...
function Chat() {
  ...
  const [chatJSON, setChat] = useState([{type:"sent", msg:"Hello, how can we help you?"}]);
  ...
  const getChat = (m) => {
    ...
    fetch("http://localhost/canteenBack/websocket.php?from=admin").then((res)=> {
      res.json().then(d=> {
        //console.log(d);
        if(!d) return;
        //if(d.no === temp[temp.length-1].no) return;
        d.forEach(z=> {
          /*z.from === "user" && */temp.push({type:"recv", msg:z.msg});
        })
        setChat(temp);
      });
    });
